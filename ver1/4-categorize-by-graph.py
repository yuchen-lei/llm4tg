...
address_category_map = {
    split_category_address(x)[1]: split_category_address(x)[0]
...
async def main():
    ...
    df_ref = df.groupby("label").sample(n=3, random_state=42,
                                        replace=False)[["account", "label"]]
    ref_graph_builder = []
    ...
    results = {}
    for _, current_graph in tqdm(test_graphs.iterrows(),
        ...
        with open("4-categorize-by-graph-gpt4-p2.pkl", "wb") as f:
            pickle.dump(results, f)
        with open("4-categorize-by-graph-gpt4-p2.json", "w") as f:
