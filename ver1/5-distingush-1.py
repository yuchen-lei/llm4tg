...
exchange_address = [
    x
    for x in address_category_map
    if address_category_map[x] == "exchange" and x in address_has_features
]

laundering_addresses = random.sample(laundering_addresses, 10)
exchange_address = random.sample(exchange_address, 10)

ref_graphs = laundering_addresses + exchange_address
...
with open("5-distingush-1-graph-prompt.txt", "w") as f:
    f.write(graph_prompt_repr)
with open("5-distingush-1-graph-index.txt", "w") as f:
    f.write(
...
async def main():
    llm = get_llm_model()
    task = await distingush_graphs(graph_prompt_repr, llm)
    with open("5-distingush-1.json", "w") as f:
